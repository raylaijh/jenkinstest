...
if [ "$#" -ne 3 ]; then
    echo "Usage:"
    echo "  $0 GUID USER FROM_JENKINS e.g. ./bin/setup_projects.sh 1111 opentlc-mgr false"
    exit 1
fi
...
echo "Creating Jenkins/Dev/Prod Projects for GUID=${GUID} and USER=${USER}"
oc new-project ${GUID}-jenkins    --display-name="${GUID} AdvDev Jenkins"
oc new-project ${GUID}-tasks-dev  --display-name="${GUID} AdvDev Tasks Development"
oc new-project ${GUID}-tasks-prod --display-name="${GUID} AdvDev Tasks Production"
